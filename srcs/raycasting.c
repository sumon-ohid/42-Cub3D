...
void	add_image_pixel(t_data *data, t_point point, int x_coord, double y_coord)
{
    double	texture_x;
    double	texture_y;
    int		index;
    ...
    while (++index < WIN_H / 2 + x_coord && index < WIN_H)
    {
        if ((int)texture_x == 0)
            data->img->pixels[WIN_W * index + data->ray_num]
                = data->ea_img->pixels[(int)y_coord * IMAGE_SIZE + (int)texture_y];
        else if ((int)texture_y == 0)
            data->img->pixels[WIN_W * index
                + data->ray_num] = data->so_img->pixels[(int)y_coord * IMAGE_SIZE
                + IMAGE_SIZE - 1 - (int)texture_x];
        else if ((int)texture_x == IMAGE_SIZE - 1)
            data->img->pixels[WIN_W * index
                + data->ray_num] = data->we_img->pixels[(int)y_coord * IMAGE_SIZE
                + IMAGE_SIZE - 1 - (int)texture_y];
        else if ((int)texture_y == IMAGE_SIZE - 1)
            data->img->pixels[WIN_W * index + data->ray_num]
                = data->no_img->pixels[(int)y_coord * IMAGE_SIZE + (int)texture_x];
...
{
    if (data->map[(int)point1.y / IMAGE_SIZE]
        [(int)point1.x / IMAGE_SIZE] == EMPTY)
        point1.x -= dx;
    draw_texture(data, sqrt(pow(data->player.y - point1.y, 2) + pow(data->player.x
