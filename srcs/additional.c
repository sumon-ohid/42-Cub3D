...
void	cub_minimap(t_data *data)
{
    ...
    while (++y < data->map_height)
    {
        ...
        {
            if (data->map[y][x] == '1')
                draw_cell(data, x * MMAP_SIZE, (y * MMAP_SIZE) + start_y, MMAP_WALL);
            else if (data->map[y][x] == '0')
                draw_cell(data, (x * MMAP_SIZE), (y * MMAP_SIZE) + start_y, MMAP_EMPTY);
        }
    }
    draw_cell(data, data->player.x / IMAGE_SIZE * MMAP_SIZE, 
        (data->player.y / IMAGE_SIZE * MMAP_SIZE) + start_y, MMAP_PLAYER);
}
